...
@Component({
  ...
  template: `
    <div class="form-card">
      ...
      <!-- ── Google Sign-In (GSI SDK — real OAuth popup) ── -->
      ...
      <div class="form-meta">
        <label class="check-lbl">
          <input type="checkbox" [(ngModel)]="remember" name="remember"/> Remember me
        </label>
      ...
      @if (backendError()) {
        <div class="error-banner">{{ backendError() }}</div>
      }

      <button class="cta-btn" (click)="submit()" [disabled]="emailLoading()">
        @if (emailLoading()) { Signing in… } @else { Sign In }
...
export class LoginComponent {
  ...
  submit(): void {
    ...
    this.http.post<{ status: number; token: string }>(
      ...
      next: (res) => {
        ...
        // Store the JWT so every subsequent request can use it
        localStorage.setItem('qma_jwt', res.token);

        // Update the AuthService user signal so guards and layout work
        this.auth.setUserFromBackend(this.username);
  ...
  async googleSignIn(): Promise<void> {
    try {
      ...
      this.toast.show(this.googleErrMsg(err), 'error');
    }
  }

  private googleErrMsg(err: unknown): string {
    const msg = (err as Error)?.message ?? '';
    if (msg === 'Google GSI SDK not loaded. Check your internet connection.') return msg;
    if (msg === 'PARSE_ERROR') return 'Could not read Google account info. Please try again.';
