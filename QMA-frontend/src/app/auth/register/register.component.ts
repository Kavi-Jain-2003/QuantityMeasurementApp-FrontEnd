...
@Component({
  ...
  template: `
    <div class="form-card">
      ...
      <!-- Username — this is what the backend stores -->
      <div class="field">
      ...
      <!-- Email (display/UX only — not required by this backend) -->
      <div class="field">
    ...
    this.http.post(
      ...
      next: (responseText: string) => {
        ...
        if (responseText.toLowerCase().includes('already exists')) {
          this.backendError.set('Username already taken. Please choose another.');
          this.toast.show('Username already taken.', 'error');
          return;
        }

        // Success — navigate to login so they get a JWT token
        this.toast.show('Account created — please sign in! 🎉', 'success');
        this.router.navigate(['/auth/login']);
      },
      error: (err) => {
        this.emailLoading.set(false);
        // ✅ Safely extract error message — never show [object ProgressEvent]
